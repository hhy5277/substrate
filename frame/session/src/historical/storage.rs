...
fn derive_key<P: AsRef<[u8]>, T: Trait>(prefix: P, session_index: &[u8]) -> Vec<u8> {
    ...
    (&mut concatenated[(prefix.len() + 1)..]).extend_from_slice(session_index);
    concatenated
}
...
impl<T: Trait> ValidatorSet<T> {
    ...
    fn prune_older_than(first_to_keep: SessionIndex) {
        let derived_key = derive_key(LAST_PRUNE, "---");
